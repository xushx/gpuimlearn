...
# Name : imECOC
#
# Purpose : imECOC is an classification algorithm for multi-imbalanced data.
# 
...
class ImECOC:
    ...
    def run(self, x_train, x_test, y_train, y_test):
        ...
        code, ft, labels = self.funClassifier(x_train, y_train, type)
        W = self.funcW(x_train, y_train, code, ft, labels)
        if withw == 0:
            W[0:len(ft)] = [1] * len(ft)

        pre = self.funcPre(x_test, code, ft, W)
