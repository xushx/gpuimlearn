...
# Name : imECOC
#
# Purpose : imECOC is an classification algorithm for multi-imbalanced data, which applies the focal loss in Boosting method.
#
...
class imECOC(object):
    ...
    def funClassifier(self, x_train, y_train, type):
        ...
        for t in range(num1):
            ...
            clf_cart.fit(np.array(Dt), np.array(DtLabel).ravel())
            ft.append(clf_cart)
